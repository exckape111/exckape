...
(function glassBrowser(){
 ...
 cards.forEach((card,i)=>{
  ...
  button.onclick=()=>{if(selected!==id&&!state[id].on){visited.add(id);state[id].on=true;sync();render()}open(i)};sync();
 });
 $('#reset').onclick=()=>{for(const [id,,knobs] of defs){state[id].on=false;const checkbox=$('#'+id),card=document.querySelector('[data-effect="'+id+'"]'),content=document.querySelector('.drawer-content[data-id="'+id+'"]');if(checkbox)checkbox.checked=false;if(card)card.classList.remove('active','selected');if(content){content.querySelector('.knobs').inert=false;content.querySelector('.apply-toggle').textContent='Apply effect';content.querySelector('.apply-toggle').setAttribute('aria-pressed','false')}for(const [key,,value] of knobs){state[id][key]=value;const input=$('#'+id+'-'+key),output=$('#'+id+'-'+key+'-val');if(input)input.value=value;if(output)output.value=value}}selected=null;cards.forEach(card=>card.querySelector('.tile-button').setAttribute('aria-expanded','false'));$('#ink').value='#f5f5f7';$('#paper').value='#111111';$('#transparent').checked=false;original=false;$('#compare').textContent='покажи оригинал';$('#compare').setAttribute('aria-pressed','false');panels.forEach(panel=>{panel.style.height='0px';panel.inert=true;panel.setAttribute('aria-hidden','true')});render()};
