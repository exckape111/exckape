...
const transparent=false;ctx.clearRect(0,0,w,h);ctx.fillStyle=paperColor;ctx.fillRect(0,0,w,h)
...
(function glassBrowser(){
 ...
 cards.forEach((card,i)=>{
  ...
  const checkbox=card.querySelector('input[type=checkbox]'),knobs=card.querySelector('.knobs');const button=document.createElement('button');button.type='button';button.className='tile-button';button.setAttribute('aria-expanded','false');button.setAttribute('aria-controls',panel.id);button.innerHTML='<span class="tile-name">'+name+'</span><span class="tile-applied" aria-hidden="true">применено</span>';card.prepend(button);
  const content=document.createElement('div');content.className='drawer-content';content.dataset.id=id;content.hidden=true;const head=document.createElement('div');head.className='drawer-heading';head.innerHTML='<span>'+name+'</span>';const toggle=document.createElement('button');toggle.type='button';toggle.className='apply-toggle';function sync(){toggle.textContent=state[id].on?'применено · отключить':'применить эффект';toggle.setAttribute('aria-pressed',String(state[id].on));button.setAttribute('aria-pressed',String(state[id].on));card.classList.toggle('active',state[id].on);checkbox.checked=state[id].on}toggle.onclick=()=>{state[id].on=!state[id].on;sync();render()};head.append(toggle);content.append(head,knobs);knobs.inert=false;panel.firstElementChild.append(content);
  button.onclick=()=>{if(selected!==id&&!state[id].on){state[id].on=true;sync();render()}open(i)};sync();
 });
 $('#reset').onclick=()=>{for(const [id,,knobs] of defs){state[id].on=false;const checkbox=$('#'+id),card=document.querySelector('[data-effect="'+id+'"]'),content=document.querySelector('.drawer-content[data-id="'+id+'"]');if(checkbox)checkbox.checked=false;if(card)card.classList.remove('active','selected');if(content){content.querySelector('.knobs').inert=false;content.querySelector('.apply-toggle').textContent='применить эффект';content.querySelector('.apply-toggle').setAttribute('aria-pressed','false')}for(const [key,,value] of knobs){state[id][key]=value;const input=$('#'+id+'-'+key),output=$('#'+id+'-'+key+'-val');if(input)input.value=value;if(output)output.value=value}}selected=null;cards.forEach(card=>{const button=card.querySelector('.tile-button');button.setAttribute('aria-expanded','false');button.setAttribute('aria-pressed','false')});original=false;$('#compare').textContent='покажи оригинал';$('#compare').setAttribute('aria-pressed','false');panels.forEach(panel=>{panel.style.height='0px';panel.inert=true;panel.setAttribute('aria-hidden','true')});render()};
